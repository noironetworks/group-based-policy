...
"""sg_rule remote_ips conversion

Revision ID: 7eaf537d927f
Revises: f28141ea1bbf
Create Date: 2020-04-30 00:00:00.000000
...
def upgrade():
    ...
    if 'aim_tenants' in insp.get_table_names():
        try:
            ...
            data_migrations.do_sg_rule_remote_ips_conversion(session)
        except ImportError:
            util.warn("AIM schema present, but failed to import AIM libraries"
                      " - SG rules remote_ips not converted.")
        except Exception as e:
            util.warn("Caught exception converting SG rules remote_ips: %s"
                      % e)
