...
class ApicRpcHandlerMixin(object):
    ...
    def _build_qos_details(self, info):
        for qos in info['qos_info']:
            LOG.info("qos-space: %s", self.name_mapper.project(None, qos.project_id))
            LOG.info("qos-name: %s", qos.qos_id)
        return (
            [{'policy-space': self.name_mapper.project(None, qos.project_id),
              'name': qos.qos_id} for qos in info['qos_info']])
