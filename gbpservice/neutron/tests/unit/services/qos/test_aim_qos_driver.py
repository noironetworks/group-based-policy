    ...
    def test_create_delete_bandwidth_rules(self):
        ...
        pol = self.aim_mgr.get(
            ...
            aim_res.QosRequirement(
                ...
                tenant_name=tenant_name))
        self.assertIsNotNone(pol)
        self.assertEqual(pol.egress_bw_limit, self.rule.id)
        pol = self.aim_mgr.get(
            self._aim_context,
            aim_res.QosBandwidthLimit(
                name=self.rule.id,
                tenant_name=tenant_name))
        self.assertIsNotNone(pol)
        self.assertEqual(pol.burst, self.rule.max_burst_kbps)
        self.assertEqual(pol.rate, self.rule.max_kbps)
        ...
        pol = self.aim_mgr.get(
            ...
            aim_res.QosRequirement(
                name=_policy.id,
                tenant_name=tenant_name))
        self.assertIsNone(pol.egress_bw_limit)
        pol = self.aim_mgr.get(
            self._aim_context,
            aim_res.QosBandwidthLimit(
                name=self.rule.id,
